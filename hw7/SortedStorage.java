...
public class SortedStorage<T extends Comparable<T>> implements StorageAnalyser, Comparable<SortedStorage<T>>{
    ...
    public boolean delete(Comparable val){
        Node<T> target = findNodeInList((T)val);
    ...
    public boolean find(Comparable val){
        if (val == null){
            ...
            if (StorageAnalyser.ofSameType(this, val)){
                ...
                //to Zero
                while(currNode.hasNext() && (currNode.compareTo((T)val) < 0)){
                    currNode = currNode.next();
                }
                // If the compareTo returns zero, the item has been found
                if (currNode.compareTo((T)val) == 0){
                    return true;
                }
     ...
     * @author Shreesh Tripathi
     */
    public boolean add(Comparable new_val){
        ...
        if (this.length == 0){
            this.head = new Node<T>((T)new_val);
        ...
        // StorageAnalyser class
            ...
            while ((currNode.compareTo((T)new_val) < 0) && currNode.hasNext()){
                currNode = currNode.next();
            }
            // When the position is found
            temp = new Node<T>((T)new_val);
            // Do not add the item if it already exists in the list
            if (currNode.compareTo((T)new_val) == 0){
                if (this.isSet){
                    return false;
                }
                else{
